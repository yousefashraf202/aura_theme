# Aura Theme — Login page controller
# License: MIT
...
from frappe.utils.oauth import (
    get_oauth2_authorize_url, get_oauth_keys,
    login_via_oauth2, login_via_oauth2_id_token,
...
def get_context(context):
    ...
    # Use Aura Theme Settings login logo, fallback to bundled asset
    context["logo"] = (
        frappe.db.get_single_value("Aura Theme Settings", "login_page_logo")
        or "/assets/aura_theme/images/datavalue-new-logo.svg"
    )
    context["app_name"] = (
    ...
    providers = [
        i.name for i in frappe.get_all(
            "Social Login Key",
...
def login_via_token(login_token):
    sid = frappe.cache().get_value(
        "login_token:{0}".format(login_token), expires=True
    )
    if not sid:
